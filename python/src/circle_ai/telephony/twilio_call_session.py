# twilio_call_session.py
#
# Port of CircleAI.Telephony.Twilio/TwilioCallSession.cs (C# — the EXACT spec).
#
# (3.3.0) ICallSession backed by a host-supplied IMediaStream. The carrier's
# responsibility is to terminate calls via REST + adapt transfer + DTMF send.
# Audio in/out delegate to the media stream.
#
# C# event EventHandler<CallStatus> -> add_/remove_status_changed callback pair
# taking (sender, CallStatus). C# ``_media.StatusChanged +=`` -> add_status_changed.
# The Status getter reproduces the C# "sticky non-Ringing local status wins over a
# still-Ringing media status" rule verbatim.
...
class TwilioCallSession(ICallSession):
    """(3.3.0) :class:`ICallSession` wrapping a Twilio media stream.

    Mirrors ``CircleAI.Telephony.Twilio.TwilioCallSession``.
    """

    def __init__(
        self,
        media: IMediaStream,
        carrier: "object",  # TwilioCarrier — loose to avoid an import cycle
    ...
    def status(self) -> CallStatus:
        # C#: media Ringing but a non-Ringing local status set -> local wins.
    ...
    async def send_dtmf_async(self, digits: str, *, ct: Optional[object] = None) -> None:
        if not digits:
            return
        # Prefer carrier-native out-of-band DTMF; else in-band tones over audio.
        ...
        *,
        ...
        if mode == TransferMode.WARM:
            if (
                ...
                return
            # Warm requested but no briefing pipeline configured — fall through to
            # cold transfer (best-effort) so the caller still reaches a human.

        transfer_twiml = f"<Response><Dial>{_html_encode(target_number)}</Dial></Response>"
        await self._carrier.redirect_call_async(self.info.call_id, transfer_twiml, ct=ct)
    ...
    async def hang_up_async(self, *, ct: Optional[object] = None) -> None:
        ...
        try:
            ...
            pass  # media may already be closed
...
class PendingMediaStream(IMediaStream):
    """(3.3.0) :class:`IMediaStream` for the moment between "carrier accepted
    dial" and "host's WebSocket attached." Yields no audio. Calling send before
    attach raises a friendly error. Mirrors the C# ``PendingMediaStream``.
    """
    ...
    async def send_audio_async(self, frame: AudioFrame, *, ct: Optional[object] = None) -> None:
        raise RuntimeError(
            "Cannot send audio before the host's WebSocket has attached its "
            "IMediaStream. Wire CircleAI.Hosting.Telephony.Twilio to complete "
            "the connection."
        )
...
def _html_encode(text: str) -> str:
    """C# ``System.Net.WebUtility.HtmlEncode`` for the TwiML text nodes we build."""
    import html

    return html.escape(text, quote=True)


__all__ = ["TwilioCallSession", "PendingMediaStream"]
