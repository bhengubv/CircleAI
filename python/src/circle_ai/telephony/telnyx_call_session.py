# telnyx_call_session.py
#
# Port of CircleAI.Telephony.Telnyx/TelnyxCallSession.cs (C# — the EXACT spec).
#
# (3.3.0) ICallSession backed by a host-supplied IMediaStream wired to a Telnyx
# Media Streaming WebSocket. The carrier handles termination + transfer via Call
# Control REST actions. Structurally identical to TwilioCallSession except cold
# transfer routes through the carrier's transfer_call_async (Telnyx Call Control
# /actions/transfer) rather than a TwiML redirect.
...
class TelnyxCallSession(ICallSession):
    """(3.3.0) :class:`ICallSession` wrapping a Telnyx media stream.

    Mirrors ``CircleAI.Telephony.Telnyx.TelnyxCallSession``.
    """

    def __init__(
        self,
        media: IMediaStream,
        carrier: "object",  # TelnyxCarrier — loose to avoid an import cycle
...
class TelnyxPendingMediaStream(IMediaStream):
    """(3.3.0) Pending media stream returned while the host's WebSocket attaches.

    Mirrors the C# ``TelnyxPendingMediaStream``.
    """
...
__all__ = ["TelnyxCallSession", "TelnyxPendingMediaStream"]
