...
| A5 | **Search across everything** | **BUILT, for the path that needs no model.** `LexicalIndex` + `Bm25` — one index over many sources (`SearchOrigin` carries the kind), one query, one ranked list, filterable by source. Arithmetic only, so it runs on a handset with **nothing downloaded** — the same argument as the procedural music bed. The semantic path still waits on an embedding model (A″). **And the existing `SearchScoring.SimpleRelevance` had two real faults, measured by running both scorers over the same documents**: no IDF at all (it ranks "the meeting" on *"the"*), and length divided out entirely at b=1.0 (a two-word note beats a 700-word transcript that actually discusses the subject — the failure that matters on a phone, where the long documents are the transcripts). A third claimed fault, no saturation, was **wrong** — a term-frequency rate saturates on its own. Still unwired to a screen. |
