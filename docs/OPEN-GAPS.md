...
| A5 | **Search across everything** | **The mechanism is built; there is almost nothing to search.** `LexicalIndex` + `Bm25` give one ranked query over many sources, arithmetic only, running with nothing downloaded. But audited afterwards: **memory already searches itself well** — `Recall.ForAsync` ranks by `Score(atom, situation, now) + Found(position)` with wear and staleness, so `DeviceMemory`'s `.Take(limit)` takes from an already-ranked list (an earlier note here implying otherwise was wrong) — and **no transcript store and no document store exist**. A transcript is shown, optionally saved as an `.srt` the person chooses a location for, and discarded. So "across everything" today means "across memory", which has a better search of its own. **The blocker is persistence, not search**, and where a person's transcripts live is a privacy decision rather than a coding one. |
