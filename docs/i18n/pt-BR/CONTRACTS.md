# Circle AI — Contratos Portáteis do Núcleo

Este documento define os **contratos de interface completos e agnósticos de linguagem** para o
núcleo portátil do Circle AI. Toda implementação de linguagem deve seguir esses contratos exatamente.
Use este documento como a única fonte da verdade ao fazer o port para uma nova linguagem.

---

## Índice

1. [Visão geral dos módulos](#module-overview)
...
9. [sync](#module-sync)
10. [Mapeamento de tipos](#type-mapping)

---

## Visão geral dos módulos

| Módulo | Tipos principais | Interfaces principais |
|--------|-----------|----------------|
...
Uma única mensagem em um encadeamento de conversa.
...
Evento de progresso emitido durante o download do modelo.
...
Estado emocional/de engajamento do B!. Cinco dimensões float, todas fixadas em `[0.0, 1.0]`.
**Os cálculos matemáticos devem ser idênticos ao nível de byte em todas as implementações.** Veja `docs/MEMORY_SPEC.md`.
...
Uma troca conversacional registrada.
...
A reação de um usuário a uma resposta específica do B!.
...
O estilo de comunicação evolutivo do B! para um usuário específico.
...
Um objetivo do usuário que o B! rastreia e auxilia proativamente.
...
Retorna um `AffectState` padrão recém-criado quando `userId` não é encontrado.
As implementações devem ser seguras contra falhas (write-then-swap ou equivalente).
...
`SearchAsync`: quando `queryEmbedding` for nulo, recorre à recência (os `topK` mais recentes).
...
A chave de persona unificada que acompanha a pessoa em todos os dispositivos.
...
Sentinela estática: `LanguageTag.Unknown = LanguageTag("", "Unknown", "Unknown", Latin, false, "")`.
...
Registro estático — 20 idiomas. Veja `fixtures/language_tags.json` para o conjunto completo.
Toda implementação deve expor todas as 20 tags e a lista `All` na ordem de declaração.
...
`DetectAsync` retorna `LanguageTag.Unknown` com `Confidence = 0` quando a detecção falha.
...
Uma troca completa no histórico da conversa.
...
| Tipo do contrato | C# | Rust | Go | Python | TypeScript | Kotlin | Swift | C | ArkTS |
|---|---|---|---|---|---|---|---|---|---|
