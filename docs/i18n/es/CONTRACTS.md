# Circle AI — Contratos Portables del Núcleo

Este documento define los **contratos de interfaz completos e independientes del idioma**
para el núcleo portable de Circle AI. Cada puerto de idioma debe implementar estos
contratos exactamente. Úselo como la fuente única de verdad al portar a un nuevo idioma.

---

## Tabla de contenidos

1. [Descripción general de módulos](#module-overview)
...
9. [sync](#module-sync)
10. [Mapeo de tipos](#type-mapping)

---

## Descripción general de módulos

| Módulo | Tipos clave | Interfaces clave |
|--------|-----------|----------------|
...
Un único mensaje en un hilo de conversación.
...
Evento de progreso emitido durante la descarga del modelo.
...
El estado emocional/de compromiso de B!. Cinco dimensiones float, todas restringidas a
`[0.0, 1.0]`. **Las matemáticas deben ser byte-idénticas en todas las implementaciones.**
Ver `docs/MEMORY_SPEC.md`.
...
Un intercambio conversacional registrado.
...
La reacción de un usuario a una respuesta específica de B!.
...
El estilo de comunicación en evolución de B! para un usuario específico.
...
Un objetivo de usuario que B! rastrea y con el que asiste de forma proactiva.
...
Retorna un `AffectState` predeterminado nuevo cuando no se encuentra `userId`.
Las implementaciones deben ser seguras ante fallos (escritura seguida de intercambio o
equivalente).
...
`SearchAsync`: cuando `queryEmbedding` es nulo, recurre a recencia (los `topK` más
recientes).
...
La clave de persona unificada que viaja con la persona a través de todos los dispositivos.
...
Centinela estático: `LanguageTag.Unknown = LanguageTag("", "Unknown", "Unknown", Latin, false, "")`.
...
Registro estático — 20 idiomas. Ver `fixtures/language_tags.json` para el conjunto
completo. Cada implementación debe exponer las 20 etiquetas y la lista `All` en el orden
de declaración.
...
`DetectAsync` retorna `LanguageTag.Unknown` con `Confidence = 0` cuando la detección
falla.
...
Un intercambio completado en el historial de conversación.
...
| Tipo del contrato | C# | Rust | Go | Python | TypeScript | Kotlin | Swift | C | ArkTS |
|---|---|---|---|---|---|---|---|---|---|
