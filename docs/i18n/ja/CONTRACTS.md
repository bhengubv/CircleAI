# Circle AI — ポータブルコアコントラクト

このドキュメントは、Circle AI ポータブルコアの **完全で言語非依存のインターフェースコントラクト** を定義します。すべての言語ポートはこれらのコントラクトを正確に実装しなければなりません。新しい言語へのポーティング時には、これを唯一の情報源として使用してください。
...
9. [sync](#module-sync)
10. [型マッピング](#type-mapping)

---

## モジュール概要

| モジュール | 主要な型 | 主要なインターフェース |
|--------|-----------|----------------|
...
B! の感情/エンゲージメント状態。5 つの float 次元、すべて `[0.0, 1.0]` にクランプ。
**すべての実装でバイト単位の同一性が必要です。** `docs/MEMORY_SPEC.md` を参照してください。
...
特定のユーザーに対する B! の進化するコミュニケーションスタイル。
...
`userId` が見つからない場合は、新しいデフォルトの `AffectState` を返します。
実装はクラッシュセーフでなければなりません（書き込み後スワップまたは同等の方法）。
...
`SearchAsync`: `queryEmbedding` が null の場合は、最新性（最新の `topK`）にフォールバックします。
...
すべてのデバイスをまたいでその人と共に移動する統一ペルソナキー。
...
静的センチネル: `LanguageTag.Unknown = LanguageTag("", "Unknown", "Unknown", Latin, false, "")`。
...
静的レジストリ — 20 言語。完全なセットは `fixtures/language_tags.json` を参照してください。
すべての実装は、20 のタグすべてと `All` リストを宣言順で公開しなければなりません。
...
`DetectAsync` は、検出が失敗した場合に `Confidence = 0` の `LanguageTag.Unknown` を返します。
...
| コントラクト型 | C# | Rust | Go | Python | TypeScript | Kotlin | Swift | C | ArkTS |
|---|---|---|---|---|---|---|---|---|---|
