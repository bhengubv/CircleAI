# Circle AI — Contrats portables du noyau

Ce document définit les **contrats d'interface complets et indépendants du langage**
pour le noyau portable de Circle AI. Chaque portage linguistique doit implémenter
ces contrats exactement. Utilisez ce document comme source de vérité unique lors du
portage vers un nouveau langage.

---

## Table des matières

1. [Vue d'ensemble des modules](#module-overview)
...
9. [sync](#module-sync)
10. [Correspondance des types](#type-mapping)

---

## Vue d'ensemble des modules

| Module | Types clés | Interfaces clés |
|--------|------------|-----------------|
...
Un message unique dans un fil de conversation.
...
Événement de progression émis pendant le téléchargement du modèle.
...
L'état émotionnel/d'engagement de B!. Cinq dimensions float, toutes clampées à
`[0.0, 1.0]`. **Les calculs doivent être bit à bit identiques dans toutes les
implémentations.** Voir `docs/MEMORY_SPEC.md`.
...
Un échange conversationnel enregistré.
...
La réaction d'un utilisateur à une réponse spécifique de B!.
...
Le style de communication évolutif de B! pour un utilisateur spécifique.
...
Un objectif utilisateur que B! suit et avec lequel il aide proactivement.
...
Retourne un `AffectState` par défaut quand `userId` n'est pas trouvé.
Les implémentations doivent être résistantes aux pannes (écriture puis swap ou
équivalent).
...
`SearchAsync` : quand `queryEmbedding` est null, se replie sur la récence (les `topK`
plus récents).
...
La clé de persona unifiée qui accompagne la personne sur tous ses appareils.
...
Sentinelle statique : `LanguageTag.Unknown = LanguageTag("", "Unknown", "Unknown", Latin, false, "")`.
...
Registre statique — 20 langues. Voir `fixtures/language_tags.json` pour l'ensemble
complet. Chaque implémentation doit exposer les 20 tags et la liste `All` dans l'ordre
de déclaration.
...
`DetectAsync` retourne `LanguageTag.Unknown` avec `Confidence = 0` en cas d'échec de
détection.
...
Un échange complet dans l'historique de conversation.
...
| Type du contrat | C# | Rust | Go | Python | TypeScript | Kotlin | Swift | C | ArkTS |
|---|---|---|---|---|---|---|---|---|---|
