# Circle AI — 이식 가능 코어 계약

이 문서는 Circle AI 이식 가능 코어의 **완전하고 언어 독립적인 인터페이스 계약**을 정의합니다.
모든 언어 포트는 이 계약을 정확히 구현해야 합니다. 새로운 언어로 포팅할 때 이 문서를 단일
진실의 원천으로 사용하십시오.
...
B!의 감정/참여 상태입니다. 다섯 개의 float 차원이며, 모두 `[0.0, 1.0]`으로 고정됩니다.
**수학 연산은 모든 구현에서 바이트 단위로 동일해야 합니다.** `docs/MEMORY_SPEC.md`를 참조하십시오.
...
특정 사용자에 대해 발전하는 B!의 커뮤니케이션 스타일입니다.
...
`userId`를 찾을 수 없는 경우 새로운 기본 `AffectState`를 반환합니다.
구현체는 충돌 안전해야 합니다 (쓰기 후 교환 또는 동등한 방식).
...
`SearchAsync`: `queryEmbedding`이 null인 경우, 최신순(가장 최근 `topK`)으로 폴백합니다.
...
모든 기기에서 사람과 함께 이동하는 통합 페르소나 키입니다.
...
정적 센티넬: `LanguageTag.Unknown = LanguageTag("", "Unknown", "Unknown", Latin, false, "")`.
...
정적 레지스트리 — 20개 언어. 전체 목록은 `fixtures/language_tags.json`을 참조하십시오.
모든 구현체는 선언 순서대로 20개의 태그와 `All` 목록을 노출해야 합니다.
...
`DetectAsync`는 탐지 실패 시 `Confidence = 0`인 `LanguageTag.Unknown`을 반환합니다.
...
| 계약 타입 | C# | Rust | Go | Python | TypeScript | Kotlin | Swift | C | ArkTS |
|---|---|---|---|---|---|---|---|---|---|
