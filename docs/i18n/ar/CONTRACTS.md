...
# Circle AI — عقود النواة المحمولة

يحدد هذا المستند **عقود الواجهة الكاملة المستقلة عن اللغة** للنواة المحمولة لـ Circle AI. يجب على كل منفذ لغوي تنفيذ هذه العقود بدقة. استخدم هذا المستند كمصدر الحقيقة الوحيد عند النقل إلى لغة جديدة.

---

## جدول المحتويات

1. [نظرة عامة على الوحدات](#module-overview)
...
9. [sync](#module-sync)
10. [تعيين الأنواع](#type-mapping)

---

## نظرة عامة على الوحدات

| الوحدة | الأنواع الرئيسية | الواجهات الرئيسية |
|--------|-----------|----------------|
...
حدث تقدم يُصدَر أثناء تنزيل النموذج.
...
الحالة العاطفية/التفاعلية لـ B!. خمسة أبعاد من النوع float، كلها مُقيَّدة بـ `[0.0, 1.0]`.
**يجب أن تكون الحسابات متطابقة بالبايت عبر جميع التنفيذات.** راجع `docs/MEMORY_SPEC.md`.
...
ردة فعل المستخدم على استجابة معينة من B!.
...
أسلوب التواصل المتطور لـ B! مع مستخدم محدد.
...
هدف يتتبعه B! ويساعد فيه بشكل استباقي.
...
يُعيد `AffectState` افتراضياً جديداً عندما لا يُوجَد `userId`.
يجب أن تكون التنفيذات آمنة من التعطل (كتابة-ثم-مبادلة أو ما يعادلها).
...
`SearchAsync`: عندما يكون `queryEmbedding` فارغاً، يعود إلى الحداثة (أحدث `topK`).
...
مفتاح الشخصية الموحد الذي يسافر مع الشخص عبر جميع أجهزته.
...
سنتينيل ثابت: `LanguageTag.Unknown = LanguageTag("", "Unknown", "Unknown", Latin, false, "")`.
...
سجل ثابت — 20 لغة. راجع `fixtures/language_tags.json` للمجموعة الكاملة.
يجب على كل تنفيذ أن يكشف عن جميع العلامات الـ 20 وقائمة `All` بترتيب الإعلان.
...
يُعيد `DetectAsync` الـ `LanguageTag.Unknown` مع `Confidence = 0` عند فشل الكشف.
...
تبادل مكتمل واحد في سجل المحادثة.
...
| نوع العقد | C# | Rust | Go | Python | TypeScript | Kotlin | Swift | C | ArkTS |
|---|---|---|---|---|---|---|---|---|---|
