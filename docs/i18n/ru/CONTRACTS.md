# Circle AI — Переносимые базовые контракты

Данный документ определяет **полные, языково-независимые контракты интерфейсов** для
переносимого ядра Circle AI. Каждый языковой порт должен реализовывать эти контракты
в точности. Используйте этот документ как единственный источник истины при портировании
на новый язык.
...
1. [Обзор модулей](#module-overview)
...
9. [sync](#module-sync)
10. [Отображение типов](#type-mapping)

---

## Обзор модулей

| Модуль | Ключевые типы | Ключевые интерфейсы |
|--------|-----------|----------------|
...
Одно сообщение в потоке беседы.
...
Событие хода выполнения, испускаемое во время загрузки модели.
...
Эмоционально-вовлечённостное состояние B!. Пять float-измерений, все зажатые в `[0.0, 1.0]`.
**Математика должна быть побайтово идентична во всех реализациях.** См. `docs/MEMORY_SPEC.md`.
...
Один записанный диалоговый обмен.
...
Реакция пользователя на конкретный ответ B!.
...
Развивающийся коммуникационный стиль B! для конкретного пользователя.
...
Цель пользователя, которую B! отслеживает и в достижении которой активно помогает.
...
Возвращает свежий `AffectState` по умолчанию, если `userId` не найден.
Реализации должны быть устойчивы к сбоям (запись с последующей заменой или эквивалент).
...
`SearchAsync`: если `queryEmbedding` равен null, деградирует до сортировки по давности (наиболее свежие `topK`).
...
Унифицированный ключ персоны, который сопровождает пользователя на всех устройствах.
...
Статический страж: `LanguageTag.Unknown = LanguageTag("", "Unknown", "Unknown", Latin, false, "")`.
...
Статический реестр — 20 языков. Полный набор см. в `fixtures/language_tags.json`.
Каждая реализация должна предоставлять все 20 тегов и список `All` в порядке объявления.
...
`DetectAsync` возвращает `LanguageTag.Unknown` с `Confidence = 0`, если определение не удалось.
...
Один завершённый обмен в истории беседы.
...
| Тип контракта | C# | Rust | Go | Python | TypeScript | Kotlin | Swift | C | ArkTS |
|---|---|---|---|---|---|---|---|---|---|
