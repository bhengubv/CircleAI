//! Relationships - what this vertical knows and what it will not do.
//!
...
/// What the people you are close to vertical is working with. Held on the
/// device, shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct RelationshipsDomainContext {
...
/// The people you are close to companion. Answers within its topics and
/// refuses the one thing it must, before anything else runs.
pub struct RelationshipsCompanionAdapter {
    context: RelationshipsDomainContext,
    answer: Option<Box<dyn Fn(&str, &RelationshipsDomainContext) -> String + Send + Sync>>,
}

impl RelationshipsCompanionAdapter {
    pub fn new(
        context: RelationshipsDomainContext,
        answer: Option<Box<dyn Fn(&str, &RelationshipsDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &RelationshipsDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut RelationshipsDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                RelationshipsDomainContext::REFUSES,
                RelationshipsDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                RelationshipsDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for RelationshipsCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RelationshipsCompanionAdapter")
            .field("purpose", &RelationshipsDomainContext::PURPOSE)
