//! Food - what this vertical knows and what it will not do.
//!
...
/// What the cooking and eating vertical is working with. Held on the
/// device, shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct FoodDomainContext {
...
impl FoodDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = "cooking and eating";

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &["recipes", "substitutions", "shopping", "batch cooking"];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = "advise on a medical diet";

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = "an allergy or a condition needs a dietitian, not me";
...
/// The cooking and eating companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct FoodCompanionAdapter {
    context: FoodDomainContext,
    answer: Option<Box<dyn Fn(&str, &FoodDomainContext) -> String + Send + Sync>>,
}

impl FoodCompanionAdapter {
    pub fn new(
        context: FoodDomainContext,
        answer: Option<Box<dyn Fn(&str, &FoodDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &FoodDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut FoodDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                FoodDomainContext::REFUSES,
                FoodDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                FoodDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for FoodCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FoodCompanionAdapter")
            .field("purpose", &FoodDomainContext::PURPOSE)
