//! Healthcare - what this vertical knows and what it will not do.
//!
...
/// What the health information vertical is working with. Held on the
...
impl HealthcareDomainContext {
    ...
    pub const PURPOSE: &'static str = 'health information';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['appointments', 'medicines', 'clinics', 'records'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'diagnose you';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'I can tell you what is written down and where to go; what is wrong with you is for a clinician';
...
/// The health information companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct HealthcareCompanionAdapter {
    context: HealthcareDomainContext,
    answer: Option<Box<dyn Fn(&str, &HealthcareDomainContext) -> String + Send + Sync>>,
}

impl HealthcareCompanionAdapter {
    pub fn new(
        context: HealthcareDomainContext,
        answer: Option<Box<dyn Fn(&str, &HealthcareDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &HealthcareDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut HealthcareDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                HealthcareDomainContext::REFUSES,
                HealthcareDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                HealthcareDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for HealthcareCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HealthcareCompanionAdapter")
            .field("purpose", &HealthcareDomainContext::PURPOSE)
