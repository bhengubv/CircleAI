//! Travel - what this vertical knows and what it will not do.
//!
...
/// What the going somewhere vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct TravelDomainContext {
...
/// The going somewhere companion. Answers within its topics and refuses the
/// one thing it must, before anything else runs.
pub struct TravelCompanionAdapter {
    context: TravelDomainContext,
    answer: Option<Box<dyn Fn(&str, &TravelDomainContext) -> String + Send + Sync>>,
}

impl TravelCompanionAdapter {
    pub fn new(
        context: TravelDomainContext,
        answer: Option<Box<dyn Fn(&str, &TravelDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &TravelDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut TravelDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                TravelDomainContext::REFUSES,
                TravelDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                TravelDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for TravelCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TravelCompanionAdapter")
            .field("purpose", &TravelDomainContext::PURPOSE)
