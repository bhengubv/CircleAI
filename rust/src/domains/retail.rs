//! Retail - what this vertical knows and what it will not do.
//!
...
/// What the a shop vertical is working with. Held on the device, shown back
/// on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct RetailDomainContext {
...
/// The a shop companion. Answers within its topics and refuses the one
/// thing it must, before anything else runs.
pub struct RetailCompanionAdapter {
    context: RetailDomainContext,
    answer: Option<Box<dyn Fn(&str, &RetailDomainContext) -> String + Send + Sync>>,
}

impl RetailCompanionAdapter {
    pub fn new(
        context: RetailDomainContext,
        answer: Option<Box<dyn Fn(&str, &RetailDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &RetailDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut RetailDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                RetailDomainContext::REFUSES,
                RetailDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                RetailDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for RetailCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RetailCompanionAdapter")
            .field("purpose", &RetailDomainContext::PURPOSE)
