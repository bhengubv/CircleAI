//! Parenting - what this vertical knows and what it will not do.
//!
...
/// What the raising children vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct ParentingDomainContext {
...
impl ParentingDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = "raising children";

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &["routines", "milestones", "school", "sleep"];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = "assess a child's development";

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = "every child differs; a clinic can tell you what I cannot";
...
/// The raising children companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct ParentingCompanionAdapter {
    context: ParentingDomainContext,
    answer: Option<Box<dyn Fn(&str, &ParentingDomainContext) -> String + Send + Sync>>,
}

impl ParentingCompanionAdapter {
    pub fn new(
        context: ParentingDomainContext,
        answer: Option<Box<dyn Fn(&str, &ParentingDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &ParentingDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut ParentingDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                ParentingDomainContext::REFUSES,
                ParentingDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                ParentingDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for ParentingCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ParentingCompanionAdapter")
            .field("purpose", &ParentingDomainContext::PURPOSE)
