//! Faith - what this vertical knows and what it will not do.
//!
...
/// What the religious life vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct FaithDomainContext {
...
impl FaithDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = "religious life";

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &["readings", "times", "calendars", "community"];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = "tell you what to believe";

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = "I can find the text and the times; the rest is not mine";
...
/// The religious life companion. Answers within its topics and refuses the
/// one thing it must, before anything else runs.
pub struct FaithCompanionAdapter {
    context: FaithDomainContext,
    answer: Option<Box<dyn Fn(&str, &FaithDomainContext) -> String + Send + Sync>>,
}

impl FaithCompanionAdapter {
    pub fn new(
        context: FaithDomainContext,
        answer: Option<Box<dyn Fn(&str, &FaithDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &FaithDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut FaithDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                FaithDomainContext::REFUSES,
                FaithDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                FaithDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for FaithCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FaithCompanionAdapter")
            .field("purpose", &FaithDomainContext::PURPOSE)
