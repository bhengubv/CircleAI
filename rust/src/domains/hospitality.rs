//! Hospitality - what this vertical knows and what it will not do.
//!
...
/// What the guests and venues vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct HospitalityDomainContext {
...
impl HospitalityDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = 'guests and venues';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['bookings', 'menus', 'shifts', 'reviews'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'post a review as a guest';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'a review has to be theirs';
...
/// The guests and venues companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct HospitalityCompanionAdapter {
    context: HospitalityDomainContext,
    answer: Option<Box<dyn Fn(&str, &HospitalityDomainContext) -> String + Send + Sync>>,
}

impl HospitalityCompanionAdapter {
    pub fn new(
        context: HospitalityDomainContext,
        answer: Option<Box<dyn Fn(&str, &HospitalityDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &HospitalityDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut HospitalityDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                HospitalityDomainContext::REFUSES,
                HospitalityDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                HospitalityDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for HospitalityCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HospitalityCompanionAdapter")
            .field("purpose", &HospitalityDomainContext::PURPOSE)
