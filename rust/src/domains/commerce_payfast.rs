//! Commerce.Integration.PayFast - what this vertical knows and what it will not do.
//!
...
/// What the PayFast payments vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
...
impl CommerceIntegrationPayFastDomainContext {
    ...
    pub const PURPOSE: &'static str = 'PayFast payments';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['payment links', 'settlement', 'refunds'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'take a card number';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'never type a card number to me - use the payment page';
...
/// The PayFast payments companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct CommerceIntegrationPayFastCompanionAdapter {
    context: CommerceIntegrationPayFastDomainContext,
    answer: Option<Box<dyn Fn(&str, &CommerceIntegrationPayFastDomainContext) -> String + Send + Sync>>,
}

impl CommerceIntegrationPayFastCompanionAdapter {
    pub fn new(
        context: CommerceIntegrationPayFastDomainContext,
        answer: Option<Box<dyn Fn(&str, &CommerceIntegrationPayFastDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &CommerceIntegrationPayFastDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut CommerceIntegrationPayFastDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                CommerceIntegrationPayFastDomainContext::REFUSES,
                CommerceIntegrationPayFastDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                CommerceIntegrationPayFastDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for CommerceIntegrationPayFastCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CommerceIntegrationPayFastCompanionAdapter")
            .field("purpose", &CommerceIntegrationPayFastDomainContext::PURPOSE)
