//! Creative - what this vertical knows and what it will not do.
//!
...
/// What the making things vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct CreativeDomainContext {
...
impl CreativeDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = 'making things';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['drafts', 'edits', 'briefs', 'portfolios'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = "pass someone else's work off as yours";

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'I will help you make your own';
...
/// The making things companion. Answers within its topics and refuses the
/// one thing it must, before anything else runs.
pub struct CreativeCompanionAdapter {
    context: CreativeDomainContext,
    answer: Option<Box<dyn Fn(&str, &CreativeDomainContext) -> String + Send + Sync>>,
}

impl CreativeCompanionAdapter {
    pub fn new(
        context: CreativeDomainContext,
        answer: Option<Box<dyn Fn(&str, &CreativeDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &CreativeDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut CreativeDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                CreativeDomainContext::REFUSES,
                CreativeDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                CreativeDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for CreativeCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CreativeCompanionAdapter")
            .field("purpose", &CreativeDomainContext::PURPOSE)
