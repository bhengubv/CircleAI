//! Personal.Mental - what this vertical knows and what it will not do.
//!
...
/// What the how you are doing vertical is working with. Held on the device,
...
/// The how you are doing companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct PersonalMentalCompanionAdapter {
    context: PersonalMentalDomainContext,
    answer: Option<Box<dyn Fn(&str, &PersonalMentalDomainContext) -> String + Send + Sync>>,
}

impl PersonalMentalCompanionAdapter {
    pub fn new(
        context: PersonalMentalDomainContext,
        answer: Option<Box<dyn Fn(&str, &PersonalMentalDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &PersonalMentalDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut PersonalMentalDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                PersonalMentalDomainContext::REFUSES,
                PersonalMentalDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                PersonalMentalDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for PersonalMentalCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PersonalMentalCompanionAdapter")
            .field("purpose", &PersonalMentalDomainContext::PURPOSE)
