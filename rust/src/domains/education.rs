//! Education - what this vertical knows and what it will not do.
//!
...
/// What the learning and teaching vertical is working with. Held on the
...
impl EducationDomainContext {
    ...
    pub const PURPOSE: &'static str = "learning and teaching";

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &["explanations", "practice", "marking", "timetables"];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = "write an assessment you will submit as your own";

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = "I will teach it and check your attempt";
...
/// The learning and teaching companion. Answers within its topics and
/// refuses the one thing it must, before anything else runs.
pub struct EducationCompanionAdapter {
    context: EducationDomainContext,
    answer: Option<Box<dyn Fn(&str, &EducationDomainContext) -> String + Send + Sync>>,
}

impl EducationCompanionAdapter {
    pub fn new(
        context: EducationDomainContext,
        answer: Option<Box<dyn Fn(&str, &EducationDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &EducationDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut EducationDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                EducationDomainContext::REFUSES,
                EducationDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                EducationDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for EducationCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("EducationCompanionAdapter")
            .field("purpose", &EducationDomainContext::PURPOSE)
