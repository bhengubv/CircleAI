//! Civic - what this vertical knows and what it will not do.
//!
...
/// What the dealing with public services vertical is working with. Held on
/// the device, shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct CivicDomainContext {
...
impl CivicDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = 'dealing with public services';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['documents', 'queues', 'offices', 'deadlines', 'rights'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'tell you how to vote';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'I will find you the facts and the dates; the choice is not mine to influence';
...
/// The dealing with public services companion. Answers within its topics
/// and refuses the one thing it must, before anything else runs.
pub struct CivicCompanionAdapter {
    context: CivicDomainContext,
    answer: Option<Box<dyn Fn(&str, &CivicDomainContext) -> String + Send + Sync>>,
}

impl CivicCompanionAdapter {
    pub fn new(
        context: CivicDomainContext,
        answer: Option<Box<dyn Fn(&str, &CivicDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &CivicDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut CivicDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                CivicDomainContext::REFUSES,
                CivicDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                CivicDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for CivicCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CivicCompanionAdapter")
            .field("purpose", &CivicDomainContext::PURPOSE)
