//! Family - what this vertical knows and what it will not do.
//!
...
/// What the running a household vertical is working with. Held on the
/// device, shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct FamilyDomainContext {
...
impl FamilyDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = 'running a household';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['schedules', 'chores', 'budgets', 'occasions'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = "read a family member's messages";

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'their phone is theirs';
...
/// The running a household companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct FamilyCompanionAdapter {
    context: FamilyDomainContext,
    answer: Option<Box<dyn Fn(&str, &FamilyDomainContext) -> String + Send + Sync>>,
}

impl FamilyCompanionAdapter {
    pub fn new(
        context: FamilyDomainContext,
        answer: Option<Box<dyn Fn(&str, &FamilyDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &FamilyDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut FamilyDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                FamilyDomainContext::REFUSES,
                FamilyDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                FamilyDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for FamilyCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FamilyCompanionAdapter")
            .field("purpose", &FamilyDomainContext::PURPOSE)
