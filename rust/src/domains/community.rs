//! Community - what this vertical knows and what it will not do.
//!
...
/// What the neighbourhood life vertical is working with. Held on the
/// device, shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct CommunityDomainContext {
...
/// The neighbourhood life companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct CommunityCompanionAdapter {
    context: CommunityDomainContext,
    answer: Option<Box<dyn Fn(&str, &CommunityDomainContext) -> String + Send + Sync>>,
}

impl CommunityCompanionAdapter {
    pub fn new(
        context: CommunityDomainContext,
        answer: Option<Box<dyn Fn(&str, &CommunityDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &CommunityDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut CommunityDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                CommunityDomainContext::REFUSES,
                CommunityDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                CommunityDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for CommunityCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CommunityCompanionAdapter")
            .field("purpose", &CommunityDomainContext::PURPOSE)
