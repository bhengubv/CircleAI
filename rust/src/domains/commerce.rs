//! Commerce - what this vertical knows and what it will not do.
//!
...
/// What the buying and selling vertical is working with. Held on the
...
impl CommerceDomainContext {
    ...
    pub const PURPOSE: &'static str = "buying and selling";

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &["listings", "orders", "delivery", "returns"];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = "move money";

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = "I can prepare it; sending it is yours to do";
...
/// The buying and selling companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct CommerceCompanionAdapter {
    context: CommerceDomainContext,
    answer: Option<Box<dyn Fn(&str, &CommerceDomainContext) -> String + Send + Sync>>,
}

impl CommerceCompanionAdapter {
    pub fn new(
        context: CommerceDomainContext,
        answer: Option<Box<dyn Fn(&str, &CommerceDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &CommerceDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut CommerceDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                CommerceDomainContext::REFUSES,
                CommerceDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                CommerceDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for CommerceCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CommerceCompanionAdapter")
            .field("purpose", &CommerceDomainContext::PURPOSE)
