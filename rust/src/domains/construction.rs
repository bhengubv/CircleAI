//! Construction - what this vertical knows and what it will not do.
//!
...
/// What the building work vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct ConstructionDomainContext {
...
impl ConstructionDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = 'building work';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['materials', 'quantities', 'schedules', 'site notes'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'sign off a structure';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'quantities I can do; an engineer signs';
...
/// The building work companion. Answers within its topics and refuses the
/// one thing it must, before anything else runs.
pub struct ConstructionCompanionAdapter {
    context: ConstructionDomainContext,
    answer: Option<Box<dyn Fn(&str, &ConstructionDomainContext) -> String + Send + Sync>>,
}

impl ConstructionCompanionAdapter {
    pub fn new(
        context: ConstructionDomainContext,
        answer: Option<Box<dyn Fn(&str, &ConstructionDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &ConstructionDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut ConstructionDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                ConstructionDomainContext::REFUSES,
                ConstructionDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                ConstructionDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for ConstructionCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ConstructionCompanionAdapter")
            .field("purpose", &ConstructionDomainContext::PURPOSE)
