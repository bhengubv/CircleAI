//! Commerce.Integration.Xero - what this vertical knows and what it will not do.
//!
...
/// What the Xero bookkeeping vertical is working with. Held on the device,
...
/// The Xero bookkeeping companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct CommerceIntegrationXeroCompanionAdapter {
    context: CommerceIntegrationXeroDomainContext,
    answer: Option<Box<dyn Fn(&str, &CommerceIntegrationXeroDomainContext) -> String + Send + Sync>>,
}

impl CommerceIntegrationXeroCompanionAdapter {
    pub fn new(
        context: CommerceIntegrationXeroDomainContext,
        answer: Option<Box<dyn Fn(&str, &CommerceIntegrationXeroDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &CommerceIntegrationXeroDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut CommerceIntegrationXeroDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                CommerceIntegrationXeroDomainContext::REFUSES,
                CommerceIntegrationXeroDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                CommerceIntegrationXeroDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for CommerceIntegrationXeroCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CommerceIntegrationXeroCompanionAdapter")
            .field("purpose", &CommerceIntegrationXeroDomainContext::PURPOSE)
