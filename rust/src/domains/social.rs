//! Social - what this vertical knows and what it will not do.
//!
...
/// What the keeping in touch vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct SocialDomainContext {
...
impl SocialDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = 'keeping in touch';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['posts', 'replies', 'groups', 'events'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'post as you without asking';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'I will draft it; posting is yours';
...
/// The keeping in touch companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct SocialCompanionAdapter {
    context: SocialDomainContext,
    answer: Option<Box<dyn Fn(&str, &SocialDomainContext) -> String + Send + Sync>>,
}

impl SocialCompanionAdapter {
    pub fn new(
        context: SocialDomainContext,
        answer: Option<Box<dyn Fn(&str, &SocialDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &SocialDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut SocialDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                SocialDomainContext::REFUSES,
                SocialDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                SocialDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for SocialCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SocialCompanionAdapter")
            .field("purpose", &SocialDomainContext::PURPOSE)
