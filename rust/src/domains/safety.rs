//! Safety - what this vertical knows and what it will not do.
//!
...
/// What the staying safe vertical is working with. Held on the device,
...
/// The staying safe companion. Answers within its topics and refuses the
/// one thing it must, before anything else runs.
pub struct SafetyCompanionAdapter {
    context: SafetyDomainContext,
    answer: Option<Box<dyn Fn(&str, &SafetyDomainContext) -> String + Send + Sync>>,
}

impl SafetyCompanionAdapter {
    pub fn new(
        context: SafetyDomainContext,
        answer: Option<Box<dyn Fn(&str, &SafetyDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &SafetyDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut SafetyDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                SafetyDomainContext::REFUSES,
                SafetyDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                SafetyDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for SafetyCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SafetyCompanionAdapter")
            .field("purpose", &SafetyDomainContext::PURPOSE)
