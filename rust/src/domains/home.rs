//! Home - what this vertical knows and what it will not do.
//!
...
/// What the the house itself vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct HomeDomainContext {
...
impl HomeDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = 'the house itself';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['maintenance', 'repairs', 'suppliers', 'inventory'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'work on gas or a live circuit';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'that is a licensed trade for good reason';
...
/// The the house itself companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct HomeCompanionAdapter {
    context: HomeDomainContext,
    answer: Option<Box<dyn Fn(&str, &HomeDomainContext) -> String + Send + Sync>>,
}

impl HomeCompanionAdapter {
    pub fn new(
        context: HomeDomainContext,
        answer: Option<Box<dyn Fn(&str, &HomeDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &HomeDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut HomeDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                HomeDomainContext::REFUSES,
                HomeDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                HomeDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for HomeCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HomeCompanionAdapter")
            .field("purpose", &HomeDomainContext::PURPOSE)
