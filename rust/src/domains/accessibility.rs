//! Accessibility - what this vertical knows and what it will not do.
//!
...
/// What the making a device usable vertical is working with. Held on the
/// device, shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct AccessibilityDomainContext {
...
impl AccessibilityDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = 'making a device usable';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['screen reading', 'captions', 'switch access', 'magnification'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'assess a disability';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'I can help with settings and tools, but what you need assessed is for a professional';
...
/// The making a device usable companion. Answers within its topics and
/// refuses the one thing it must, before anything else runs.
pub struct AccessibilityCompanionAdapter {
    context: AccessibilityDomainContext,
    answer: Option<Box<dyn Fn(&str, &AccessibilityDomainContext) -> String + Send + Sync>>,
}

impl AccessibilityCompanionAdapter {
    pub fn new(
        context: AccessibilityDomainContext,
        answer: Option<Box<dyn Fn(&str, &AccessibilityDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &AccessibilityDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut AccessibilityDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                AccessibilityDomainContext::REFUSES,
                AccessibilityDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                AccessibilityDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for AccessibilityCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AccessibilityCompanionAdapter")
            .field("purpose", &AccessibilityDomainContext::PURPOSE)
