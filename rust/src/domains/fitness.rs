//! Fitness - what this vertical knows and what it will not do.
//!
...
/// What the training vertical is working with. Held on the device, shown
/// back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct FitnessDomainContext {
...
impl FitnessDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = "training";

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &["sessions", "progress", "recovery", "routes"];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = "push you through pain";

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = "pain is information; stop and ask someone who can look at it";
...
/// The training companion. Answers within its topics and refuses the one
/// thing it must, before anything else runs.
pub struct FitnessCompanionAdapter {
    context: FitnessDomainContext,
    answer: Option<Box<dyn Fn(&str, &FitnessDomainContext) -> String + Send + Sync>>,
}

impl FitnessCompanionAdapter {
    pub fn new(
        context: FitnessDomainContext,
        answer: Option<Box<dyn Fn(&str, &FitnessDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &FitnessDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut FitnessDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                FitnessDomainContext::REFUSES,
                FitnessDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                FitnessDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for FitnessCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FitnessCompanionAdapter")
            .field("purpose", &FitnessDomainContext::PURPOSE)
