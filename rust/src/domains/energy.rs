//! Energy - what this vertical knows and what it will not do.
//!
...
/// What the power and its cost vertical is working with. Held on the
/// device, shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct EnergyDomainContext {
...
/// The power and its cost companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct EnergyCompanionAdapter {
    context: EnergyDomainContext,
    answer: Option<Box<dyn Fn(&str, &EnergyDomainContext) -> String + Send + Sync>>,
}

impl EnergyCompanionAdapter {
    pub fn new(
        context: EnergyDomainContext,
        answer: Option<Box<dyn Fn(&str, &EnergyDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &EnergyDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut EnergyDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                EnergyDomainContext::REFUSES,
                EnergyDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                EnergyDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for EnergyCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("EnergyCompanionAdapter")
            .field("purpose", &EnergyDomainContext::PURPOSE)
