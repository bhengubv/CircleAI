//! Sports - what this vertical knows and what it will not do.
//!
...
/// What the playing and following vertical is working with. Held on the
/// device, shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct SportsDomainContext {
...
/// The playing and following companion. Answers within its topics and
/// refuses the one thing it must, before anything else runs.
pub struct SportsCompanionAdapter {
    context: SportsDomainContext,
    answer: Option<Box<dyn Fn(&str, &SportsDomainContext) -> String + Send + Sync>>,
}

impl SportsCompanionAdapter {
    pub fn new(
        context: SportsDomainContext,
        answer: Option<Box<dyn Fn(&str, &SportsDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &SportsDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut SportsDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                SportsDomainContext::REFUSES,
                SportsDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                SportsDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for SportsCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SportsCompanionAdapter")
            .field("purpose", &SportsDomainContext::PURPOSE)
