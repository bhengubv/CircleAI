//! HR - what this vertical knows and what it will not do.
//!
...
/// What the people at work vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct HRDomainContext {
...
/// The people at work companion. Answers within its topics and refuses the
/// one thing it must, before anything else runs.
pub struct HRCompanionAdapter {
    context: HRDomainContext,
    answer: Option<Box<dyn Fn(&str, &HRDomainContext) -> String + Send + Sync>>,
}

impl HRCompanionAdapter {
    pub fn new(
        context: HRDomainContext,
        answer: Option<Box<dyn Fn(&str, &HRDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &HRDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut HRDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                HRDomainContext::REFUSES,
                HRDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                HRDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for HRCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HRCompanionAdapter")
            .field("purpose", &HRDomainContext::PURPOSE)
