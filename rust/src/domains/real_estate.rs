//! RealEstate - what this vertical knows and what it will not do.
//!
...
/// What the property vertical is working with. Held on the device, shown
/// back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct RealEstateDomainContext {
...
impl RealEstateDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = "property";

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &["listings", "viewings", "documents", "prices"];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = "value a property";

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = "I can show you what sold nearby; a valuation is signed by someone qualified";
...
/// The property companion. Answers within its topics and refuses the one
/// thing it must, before anything else runs.
pub struct RealEstateCompanionAdapter {
    context: RealEstateDomainContext,
    answer: Option<Box<dyn Fn(&str, &RealEstateDomainContext) -> String + Send + Sync>>,
}

impl RealEstateCompanionAdapter {
    pub fn new(
        context: RealEstateDomainContext,
        answer: Option<Box<dyn Fn(&str, &RealEstateDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &RealEstateDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut RealEstateDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                RealEstateDomainContext::REFUSES,
                RealEstateDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                RealEstateDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for RealEstateCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RealEstateCompanionAdapter")
            .field("purpose", &RealEstateDomainContext::PURPOSE)
