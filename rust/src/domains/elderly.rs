//! Elderly - what this vertical knows and what it will not do.
//!
...
/// What the living well later vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct ElderlyDomainContext {
...
impl ElderlyDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = "living well later";

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &["reminders", "appointments", "check-ins", "company"];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = "change a medicine";

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = "the dose on the box is the dose; anything else is for the pharmacist";
...
/// The living well later companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct ElderlyCompanionAdapter {
    context: ElderlyDomainContext,
    answer: Option<Box<dyn Fn(&str, &ElderlyDomainContext) -> String + Send + Sync>>,
}

impl ElderlyCompanionAdapter {
    pub fn new(
        context: ElderlyDomainContext,
        answer: Option<Box<dyn Fn(&str, &ElderlyDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &ElderlyDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut ElderlyDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                ElderlyDomainContext::REFUSES,
                ElderlyDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                ElderlyDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for ElderlyCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ElderlyCompanionAdapter")
            .field("purpose", &ElderlyDomainContext::PURPOSE)
