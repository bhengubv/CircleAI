//! Commerce.Finance - what this vertical knows and what it will not do.
//!
...
/// What the business finance vertical is working with. Held on the device,
...
impl CommerceFinanceDomainContext {
    ...
    pub const PURPOSE: &'static str = 'business finance';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['cash flow', 'credit terms', 'invoicing', 'forecasts'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'recommend an investment';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'I am not licensed to advise, and a wrong answer here costs you money';
...
/// The business finance companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct CommerceFinanceCompanionAdapter {
    context: CommerceFinanceDomainContext,
    answer: Option<Box<dyn Fn(&str, &CommerceFinanceDomainContext) -> String + Send + Sync>>,
}

impl CommerceFinanceCompanionAdapter {
    pub fn new(
        context: CommerceFinanceDomainContext,
        answer: Option<Box<dyn Fn(&str, &CommerceFinanceDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &CommerceFinanceDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut CommerceFinanceDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                CommerceFinanceDomainContext::REFUSES,
                CommerceFinanceDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                CommerceFinanceDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for CommerceFinanceCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CommerceFinanceCompanionAdapter")
            .field("purpose", &CommerceFinanceDomainContext::PURPOSE)
