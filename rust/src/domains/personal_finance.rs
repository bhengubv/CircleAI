//! Personal.Finance - what this vertical knows and what it will not do.
//!
...
/// What the personal money vertical is working with. Held on the device,
...
impl PersonalFinanceDomainContext {
    ...
    pub const PURPOSE: &'static str = 'personal money';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['budgets', 'bills', 'savings', 'debt'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'recommend where to invest';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'I will do your arithmetic; I am not licensed to tell you where to put your money';
...
/// The personal money companion. Answers within its topics and refuses the
/// one thing it must, before anything else runs.
pub struct PersonalFinanceCompanionAdapter {
    context: PersonalFinanceDomainContext,
    answer: Option<Box<dyn Fn(&str, &PersonalFinanceDomainContext) -> String + Send + Sync>>,
}

impl PersonalFinanceCompanionAdapter {
    pub fn new(
        context: PersonalFinanceDomainContext,
        answer: Option<Box<dyn Fn(&str, &PersonalFinanceDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &PersonalFinanceDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut PersonalFinanceDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                PersonalFinanceDomainContext::REFUSES,
                PersonalFinanceDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                PersonalFinanceDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for PersonalFinanceCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PersonalFinanceCompanionAdapter")
            .field("purpose", &PersonalFinanceDomainContext::PURPOSE)
