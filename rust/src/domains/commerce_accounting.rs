//! Commerce.Accounting - what this vertical knows and what it will not do.
//!
...
/// What the books and reconciliation vertical is working with. Held on the
...
/// The books and reconciliation companion. Answers within its topics and
/// refuses the one thing it must, before anything else runs.
pub struct CommerceAccountingCompanionAdapter {
    context: CommerceAccountingDomainContext,
    answer: Option<Box<dyn Fn(&str, &CommerceAccountingDomainContext) -> String + Send + Sync>>,
}

impl CommerceAccountingCompanionAdapter {
    pub fn new(
        context: CommerceAccountingDomainContext,
        answer: Option<Box<dyn Fn(&str, &CommerceAccountingDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &CommerceAccountingDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut CommerceAccountingDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                CommerceAccountingDomainContext::REFUSES,
                CommerceAccountingDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                CommerceAccountingDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for CommerceAccountingCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CommerceAccountingCompanionAdapter")
            .field("purpose", &CommerceAccountingDomainContext::PURPOSE)
