//! Legal - what this vertical knows and what it will not do.
//!
...
/// What the the law vertical is working with. Held on the device, shown
...
impl LegalDomainContext {
    ...
    pub const PURPOSE: &'static str = "the law";

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &["documents", "deadlines", "rights", "where to go"];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = "give legal advice";

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = "I can explain what a document says and what a date means; advice is for someone admitted to practise";
...
/// The the law companion. Answers within its topics and refuses the one
/// thing it must, before anything else runs.
pub struct LegalCompanionAdapter {
    context: LegalDomainContext,
    answer: Option<Box<dyn Fn(&str, &LegalDomainContext) -> String + Send + Sync>>,
}

impl LegalCompanionAdapter {
    pub fn new(
        context: LegalDomainContext,
        answer: Option<Box<dyn Fn(&str, &LegalDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &LegalDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut LegalDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                LegalDomainContext::REFUSES,
                LegalDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                LegalDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for LegalCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LegalCompanionAdapter")
            .field("purpose", &LegalDomainContext::PURPOSE)
