//! Tourism - what this vertical knows and what it will not do.
//!
...
/// What the showing people around vertical is working with. Held on the
/// device, shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct TourismDomainContext {
...
/// The showing people around companion. Answers within its topics and
/// refuses the one thing it must, before anything else runs.
pub struct TourismCompanionAdapter {
    context: TourismDomainContext,
    answer: Option<Box<dyn Fn(&str, &TourismDomainContext) -> String + Send + Sync>>,
}

impl TourismCompanionAdapter {
    pub fn new(
        context: TourismDomainContext,
        answer: Option<Box<dyn Fn(&str, &TourismDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &TourismDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut TourismDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                TourismDomainContext::REFUSES,
                TourismDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                TourismDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for TourismCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TourismCompanionAdapter")
            .field("purpose", &TourismDomainContext::PURPOSE)
