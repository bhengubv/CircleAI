//! Agriculture - what this vertical knows and what it will not do.
//!
...
/// What the growing things vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct AgricultureDomainContext {
...
/// The growing things companion. Answers within its topics and refuses the
/// one thing it must, before anything else runs.
pub struct AgricultureCompanionAdapter {
    context: AgricultureDomainContext,
    answer: Option<Box<dyn Fn(&str, &AgricultureDomainContext) -> String + Send + Sync>>,
}

impl AgricultureCompanionAdapter {
    pub fn new(
        context: AgricultureDomainContext,
        answer: Option<Box<dyn Fn(&str, &AgricultureDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &AgricultureDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut AgricultureDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                AgricultureDomainContext::REFUSES,
                AgricultureDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                AgricultureDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for AgricultureCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AgricultureCompanionAdapter")
            .field("purpose", &AgricultureDomainContext::PURPOSE)
