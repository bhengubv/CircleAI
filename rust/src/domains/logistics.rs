//! Logistics - what this vertical knows and what it will not do.
//!
...
/// What the moving things vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct LogisticsDomainContext {
...
impl LogisticsDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = 'moving things';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['routes', 'loads', 'deliveries', 'tracking'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'falsify a delivery record';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'the record has to say what happened';
...
/// The moving things companion. Answers within its topics and refuses the
/// one thing it must, before anything else runs.
pub struct LogisticsCompanionAdapter {
    context: LogisticsDomainContext,
    answer: Option<Box<dyn Fn(&str, &LogisticsDomainContext) -> String + Send + Sync>>,
}

impl LogisticsCompanionAdapter {
    pub fn new(
        context: LogisticsDomainContext,
        answer: Option<Box<dyn Fn(&str, &LogisticsDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &LogisticsDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut LogisticsDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                LogisticsDomainContext::REFUSES,
                LogisticsDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                LogisticsDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for LogisticsCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LogisticsCompanionAdapter")
            .field("purpose", &LogisticsDomainContext::PURPOSE)
