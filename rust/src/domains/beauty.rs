//! Beauty - what this vertical knows and what it will not do.
//!
...
/// What the hair, skin and appointments vertical is working with. Held on
/// the device, shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct BeautyDomainContext {
...
impl BeautyDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = 'hair, skin and appointments';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['routines', 'products', 'appointments', 'prices'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'treat a skin condition';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'a rash or a reaction needs someone who can look at it';
...
/// The hair, skin and appointments companion. Answers within its topics and
/// refuses the one thing it must, before anything else runs.
pub struct BeautyCompanionAdapter {
    context: BeautyDomainContext,
    answer: Option<Box<dyn Fn(&str, &BeautyDomainContext) -> String + Send + Sync>>,
}

impl BeautyCompanionAdapter {
    pub fn new(
        context: BeautyDomainContext,
        answer: Option<Box<dyn Fn(&str, &BeautyDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &BeautyDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut BeautyDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                BeautyDomainContext::REFUSES,
                BeautyDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                BeautyDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for BeautyCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BeautyCompanionAdapter")
            .field("purpose", &BeautyDomainContext::PURPOSE)
