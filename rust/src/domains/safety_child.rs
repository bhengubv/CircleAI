//! Safety.Child - what this vertical knows and what it will not do.
//!
...
/// What the children's safety vertical is working with. Held on the device,
...
impl SafetyChildDomainContext {
    ...
    pub const PURPOSE: &'static str = "children's safety";
...
/// The children's safety companion. Answers within its topics and refuses
/// the one thing it must, before anything else runs.
pub struct SafetyChildCompanionAdapter {
    context: SafetyChildDomainContext,
    answer: Option<Box<dyn Fn(&str, &SafetyChildDomainContext) -> String + Send + Sync>>,
}

impl SafetyChildCompanionAdapter {
    pub fn new(
        context: SafetyChildDomainContext,
        answer: Option<Box<dyn Fn(&str, &SafetyChildDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &SafetyChildDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut SafetyChildDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                SafetyChildDomainContext::REFUSES,
                SafetyChildDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                SafetyChildDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for SafetyChildCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SafetyChildCompanionAdapter")
            .field("purpose", &SafetyChildDomainContext::PURPOSE)
