//! Kids - what this vertical knows and what it will not do.
//!
...
/// What the younger users vertical is working with. Held on the device,
/// shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct KidsDomainContext {
...
impl KidsDomainContext {
    /// What this vertical is for.
    pub const PURPOSE: &'static str = 'younger users';

    /// What it will speak to. A topic list rather than a classifier, because a
    /// list can be read by the person it applies to.
    pub const TOPICS: &'static [&'static str] = &['homework', 'stories', 'games', 'questions'];

    /// The one thing it will NOT do, however it is asked.
    pub const REFUSES: &'static str = 'talk about anything a grown-up should be there for';

    /// Why - in words for the person asking, not a policy identifier.
    pub const REFUSAL: &'static str = 'let us find someone you trust to help with that one';
...
/// The younger users companion. Answers within its topics and refuses the
/// one thing it must, before anything else runs.
pub struct KidsCompanionAdapter {
    context: KidsDomainContext,
    answer: Option<Box<dyn Fn(&str, &KidsDomainContext) -> String + Send + Sync>>,
}

impl KidsCompanionAdapter {
    pub fn new(
        context: KidsDomainContext,
        answer: Option<Box<dyn Fn(&str, &KidsDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &KidsDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut KidsDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                KidsDomainContext::REFUSES,
                KidsDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                KidsDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for KidsCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("KidsCompanionAdapter")
            .field("purpose", &KidsDomainContext::PURPOSE)
