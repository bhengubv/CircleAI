//! Wearable - what this vertical knows and what it will not do.
//!
...
/// What the what a worn device notices vertical is working with. Held on
/// the device, shown back on request, and cleared when asked.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct WearableDomainContext {
...
/// The what a worn device notices companion. Answers within its topics and
/// refuses the one thing it must, before anything else runs.
pub struct WearableCompanionAdapter {
    context: WearableDomainContext,
    answer: Option<Box<dyn Fn(&str, &WearableDomainContext) -> String + Send + Sync>>,
}

impl WearableCompanionAdapter {
    pub fn new(
        context: WearableDomainContext,
        answer: Option<Box<dyn Fn(&str, &WearableDomainContext) -> String + Send + Sync>>,
    ) -> Self {
    ...
    pub fn context(&self) -> &WearableDomainContext {
        &self.context
    }

    pub fn context_mut(&mut self) -> &mut WearableDomainContext {
        &mut self.context
    }
    ...
    pub fn handle(&self, request: &str) -> String {
        if self.context.is_refused(request) {
            return format!(
                "I will not {} - {}.",
                WearableDomainContext::REFUSES,
                WearableDomainContext::REFUSAL
            );
        }
        match &self.answer {
            ...
            None => format!(
                ...
                WearableDomainContext::PURPOSE
            ),
        }
    }
}

impl std::fmt::Debug for WearableCompanionAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("WearableCompanionAdapter")
            .field("purpose", &WearableDomainContext::PURPOSE)
