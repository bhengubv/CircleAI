...
# Every name the Rust port declares.
#
# struct, enum, trait, type alias, fn, const, static and mod — all eight,
# because a C# type has no single Rust spelling. A record became a struct; an
# interface became a trait; a static class became a MODULE of free functions, or
# a set of consts, or an impl block on a unit struct. Scanning only `struct`
# reports most of the port missing while it is sitting there.
#
# Both `pub` and private declarations are counted. A type internal to the port is
# still a type the port HAS, and the question this measures is whether the
# concept was ported — not whether it was re-exported.
types = set()
decl = re.compile(
    r"^\s*(?:pub(?:\s*\([^)]*\))?\s+)?"
    r"(?:async\s+|unsafe\s+|extern\s+\"[^\"]*\"\s+|const\s+|default\s+)*"
    r"(?:struct|enum|trait|union|type|fn|const|static|mod)\s+"
...
# A Rust MODULE is a name too, and the file that defines it usually carries no
# `mod` line of its own — `foo.rs` declares module `foo` by existing. A C#
# static class that became a whole module is present under the file's name.
for root, dirs, files in os.walk("rust/src"):
    dirs[:] = [d for d in dirs if d not in ("target", ".git")]
    for fn in files:
        if fn.endswith(".rs") and fn != "mod.rs" and fn != "lib.rs":
            types.add(fn[:-3])
        # A directory with a mod.rs is a module named for the directory.
    for d in dirs:
        types.add(d)
...
def to_snake(name):
    """PascalCase to snake_case, the way rustfmt would.

    An acronym run is kept together: `HttpCacheKey` becomes `http_cache_key`,
    not `h_t_t_p_cache_key`. Getting that wrong makes every acronym-bearing name
    read as missing.
    """
...
snake = {to_snake(t) for t in types}
screaming = {t.upper() for t in types}
...
def candidates(t, module):
    """Every name this type could legitimately have taken in the Rust port.

    What varies, and why each is accepted:

      the I prefix   Rust's convention has NO prefix on a trait — `IFarmBoard`
                     becomes `FarmBoard` — and that is what most of this port
                     did. Both are tried rather than picking a winner here,
                     because settling on one across 166 modules is a rename to
                     do deliberately, not a side effect of measuring.

      snake_case     a C# static class that became a module of free functions
                     has both the module and its members in snake_case, so
                     `DtmfToneGenerator` may be present as `dtmf_tone_generator`
                     — a module, a function, or both.

      SCREAMING      a C# class whose whole content was constants usually became
                     a `const` block, and Rust spells those in upper case.

      EventArgs      Rust has no events. Every one became a plain payload struct,
                     sometimes keeping "Event" and sometimes dropping the suffix
                     because the name already reads as a noun.

      Exception      a C# exception becomes an ERROR enum or struct, which is
                     Rust's convention rather than a rename — `FooException`
                     arrives as `FooError`.

      Null/Empty     a C# `NullFoo` is often a Rust unit struct with the same
                     name, and sometimes `NoopFoo`, which is the more idiomatic
                     spelling.
    """
    mod = module.replace(".", "")
    head = module.split(".")[0]
    ...
    for b in list(bases):
        ...
        if b.endswith("Exception"):
            ...
            bases.add(stem)
        if b.startswith("Null") and len(b) > 4 and b[4].isupper():
            bases.add("Noop" + b[4:])
            bases.add("No" + b[4:])
        # A C# `XxxServiceCollectionExtensions` has no DI container to extend
        # here; it arrives as a registration module or a `register_xxx` function.
    ...
    # The product name rides on the crate path here, so a type that spells
...
#
# A type deliberately absent is written down in rust/PARITY-EXCLUSIONS.md with a
# reason. Reading it here rather than hard-coding a list means there is exactly
...
def read_exclusions(path="rust/PARITY-EXCLUSIONS.md"):
...
for d in sorted(glob.glob("src/CircleAI.*/")):
    ...
    def present(t):
        ...
        for c in candidates(t, mod):
            if (c in types or c.lower() in low
                    or to_snake(c) in snake or c.upper() in screaming):
                return True
        return False
...
print("declared exclusions honoured: %d, renames: %d  (rust/PARITY-EXCLUSIONS.md)"
