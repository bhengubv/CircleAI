// TelnyxCallSession.cs
//
// (3.3.0) ICallSession backed by a host-supplied IMediaStream wired to
// a Telnyx Media Streaming WebSocket. The carrier handles termination
// + transfer via Call Control REST actions.
...
/// <summary>
/// (3.3.0) <see cref="ICallSession"/> wrapping a Telnyx media stream.
/// </summary>
public sealed class TelnyxCallSession : ICallSession
{
    private readonly IMediaStream _media;
    private readonly TelnyxCarrier _carrier;
    ...
    public TelnyxCallSession(IMediaStream media, TelnyxCarrier carrier)
    ...
    public TelnyxCallSession(IMediaStream media, TelnyxCarrier carrier, BriefingSynthesiser? briefingTts, Uri? bridgeStreamUrl)
    {
...
/// <summary>(3.3.0) Pending media stream returned while the host's WebSocket attaches.</summary>
internal sealed class TelnyxPendingMediaStream : IMediaStream
{
    public TelnyxPendingMediaStream(CallInfo info) { CallInfo = info; }
