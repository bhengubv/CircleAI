...
private static string E(string m)=>$"{SafetyChildDomainContext.SystemPromptSnippet}\n\n{m}";
    ...
    public Task<string> DesignSafetyConversationAsync(string childAge, string topic, CancellationToken ct=default)
        => _i.AgentAsync($"Design an age-appropriate safety conversation for {childAge} on: {topic}. Concrete examples, scripts they can use, role-play prompt.", ct);

    public Task<string> AssessOnlineRiskAsync(string platform, string childAge, string behaviour, CancellationToken ct=default)
        => _i.AgentAsync($"Assess online risk on {platform} for {childAge}-year-old showing {behaviour}. Specific risks + parent-action checklist.", ct);

    public Task<string> VerifyTrustedAdultsAsync(string contactList, CancellationToken ct=default)
        => _i.AgentAsync($"Help vet trusted-adult ring from: {contactList}. Criteria to apply, questions to ask the child.", ct);

    public Task<string> DraftSchoolNotificationAsync(string concern, string evidence, CancellationToken ct=default)
        => _i.AgentAsync($"Draft a school notification about: {concern}. Evidence: {evidence}. Calm, factual, requesting specific action.", ct);
