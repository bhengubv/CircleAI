...
private static string E(string m)=>$"{PetsDomainContext.SystemPromptSnippet}\n\n{m}";
    ...
    public Task<string> AdviseDietAsync(string species, string lifeStage, string healthNotes, CancellationToken ct=default)
        => _i.AgentAsync($"Advise diet for {lifeStage} {species}. Health notes: {healthNotes}. Cover composition, portions, transitions, treats.", ct);

    public Task<string> PlanTravelWithPetAsync(string species, string destination, string transport, CancellationToken ct=default)
        => _i.AgentAsync($"Plan {transport} travel to {destination} with {species}. Documents, crate, breaks, stress reduction.", ct);
