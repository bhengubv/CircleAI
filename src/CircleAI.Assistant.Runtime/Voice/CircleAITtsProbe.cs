...
public static class CircleAITtsProbe
{
        ...
        string storageDir, string wavPath, Action<string>? log = null, CancellationToken ct = default)
    {
        ...
        // as a crash.
        var (speaker, status) = await CircleAISpeaker.TryCreateAsync(storageDir, log, ct).ConfigureAwait(false);
        if (speaker is null)
        ...
        long? speakerOverride = null, long? langIdOverrideIn = null, long? foreignSpeakerId = null)
    {
        ...
        // way to use one. CircleAISpeaker imports sideloaded voices before it loads
        ...
        var sideload = CircleAISpeaker.SideloadFolder;
        ...
        CircleAI.Voice.PersonalRespellings? personal = null)
    {
        ...
        else
        {
            phonemizer = graphemeVoice
                ...
                : CircleAISpeaker.MobilePhonemizerFactory?.Invoke(voice);
        }
        if (phonemizer is null)
            return "on-device phonemizer not wired (CircleAISpeaker.MobilePhonemizerFactory)\n";
