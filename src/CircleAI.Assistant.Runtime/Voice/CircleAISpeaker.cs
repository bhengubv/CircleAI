...
public sealed class CircleAISpeaker : IDisposable
{
    ...
    private CircleAISpeaker(OnnxTtsEngine sa, OnnxTtsEngine? alt, string family = "za")
    {
        _sa = sa;
        _en = alt;
    ...
    // Asian accent while the greeting screen, which goes through CircleAITtsProbe and
    ...
    internal const string PreferredVoice = CircleAI.Assistant.VoiceNames.SouthAfrican;
    ...
    /// </remarks>
    internal const string EnglishVoice = CircleAI.Assistant.VoiceNames.English;
    ...
    /// </param>
    public static async Task<(CircleAISpeaker? speaker, string status)> TryCreateAsync(
        ...
        string? languageCode = null)
    {
        ...
        // Plan, not a nullable pick — see CircleAIListener. TTS likewise has no
        ...
        if (onMobile)
        {
            if (MobilePhonemizerFactory is null)
                phonemizerProblem = "on-device phonemizer not wired — set CircleAISpeaker.MobilePhonemizerFactory (the out-of-process espeak G2P client)";
        }
        else
        {
        ...
        // See CircleAIListener — MB / rate / ETA / file N of M / phase, not a bare %.
        var lastPct = -1;
        ...
        return (new CircleAISpeaker(engine, english, wanted), "ready");
    }
