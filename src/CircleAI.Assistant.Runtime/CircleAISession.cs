...
/// </summary>
public sealed class CircleAISession : IAsyncDisposable
{
    /// <remarks>
    ...
    /// <summary>
    /// IT!'s tools. Exposed so a host can show <see cref="CircleAIToolBridge.InvocationLog"/>
    /// — the difference between "the model answered" and "the model called a tool".
    /// </summary>
    public CircleAIToolBridge Tools { get; }
    ...
    /// <c>true</c> swaps in <see cref="CircleAIGenerator"/> — a canned responder that
        ...
        int maxTokens = 96)
    {
        ...
        Tools = new CircleAIToolBridge(batteryPercent);
        ...
        if (useStubBrain)
        {
            ...
            _brain = new AIService(stubOptions, generatorFactory: _ => new CircleAIGenerator());
        }
        else
        {
    ...
    /// </param>
    public readonly record struct CircleAIToolTurn(string Answer, IReadOnlyList<string> ToolsCalled);
