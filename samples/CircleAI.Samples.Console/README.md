# CircleAI.Samples.Console — a Neuron reference sample

This is the smallest complete example of building an on-device assistant on the
...
> It is a *sample*, not a product. It shows the wiring; it does not ship a brain.

## Run it

```bash
# interactive chat
dotnet run --project samples/CircleAI.Samples.Console

# scripted demo (no typing needed)
dotnet run --project samples/CircleAI.Samples.Console -- --demo
```
...
- **The concierge deciding.** Before every reply, the sample prints which "organ" the
...
3. **Persistent memory & persona** — set `AIOptions.EpisodicMemory` and
   `AIOptions.PersonaStore` so the sample remembers across restarts and adapts its tone.
