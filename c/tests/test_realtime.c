...
static void test_sample_rates(void) {
    assert(ca_rt_sample_rate_of(CA_RT_FMT_PCM16K) == 16000);
    assert(ca_rt_sample_rate_of(CA_RT_FMT_PCM24K) == 24000);
    assert(ca_rt_sample_rate_of(CA_RT_FMT_MULAW8K) == 8000);
...
static ca_rt_session_config_t cfg(ca_rt_audio_format_t fmt) {
    ca_rt_session_config_t c;
    memset(&c, 0, sizeof(c));
...
static size_t drain_events(ca_rt_session_t *s, ca_rt_event_t *buf, size_t cap) {
    size_t n = 0;
    ca_rt_event_t e;
    while (n < cap && ca_rt_session_receive_event_next(s, &e)) buf[n++] = e;
    return n;
}
...
static void test_loopback_audio(void) {
    ca_rt_session_config_t c = cfg(CA_RT_FMT_PCM24K);
    ca_rt_session_t *s = ca_rt_loopback_session_create(&c, NULL, NULL);
    assert(s);
    /* SessionId = "loop-<32 hex>". */
    const char *sid = ca_rt_session_id(s);
    ...
    ca_rt_audio_frame_t qf; memset(&qf, 0, sizeof(qf));
    qf.pcm = quiet; qf.pcm_len = sizeof(quiet); qf.format = CA_RT_FMT_PCM24K;
    assert(ca_rt_session_send_audio(s, &qf) == 0);
    assert(ca_rt_session_event_pending(s) == 0);   /* no speech transition */
    assert(ca_rt_session_audio_pending(s) == 1);   /* echoed */

    /* drain the echoed quiet frame. */
    ca_rt_audio_frame_t out;
    assert(ca_rt_session_receive_audio_next(s, &out));
    assert(out.pcm_len == sizeof(quiet) && out.format == CA_RT_FMT_PCM24K);
    ca_rt_audio_frame_free(&out);

    /* Loud frame: not silent -> SpeechStarted emitted, echoed. */
    uint8_t loud[128]; fill_loud(loud, sizeof(loud));
    ca_rt_audio_frame_t lf; memset(&lf, 0, sizeof(lf));
    lf.pcm = loud; lf.pcm_len = sizeof(loud); lf.format = CA_RT_FMT_PCM24K;
    assert(ca_rt_session_send_audio(s, &lf) == 0);
    ca_rt_event_t e;
    assert(ca_rt_session_receive_event_next(s, &e));
    assert(e.type == CA_RT_EVT_SPEECH_STARTED);
    ca_rt_event_free(&e);
    assert(ca_rt_session_audio_pending(s) == 1);
    assert(ca_rt_session_receive_audio_next(s, &out)); ca_rt_audio_frame_free(&out);

    /* Another loud frame: still speaking -> no new transition. */
    assert(ca_rt_session_send_audio(s, &lf) == 0);
    assert(ca_rt_session_event_pending(s) == 0);
    assert(ca_rt_session_receive_audio_next(s, &out)); ca_rt_audio_frame_free(&out);

    /* Back to quiet: SpeechEnded. */
    assert(ca_rt_session_send_audio(s, &qf) == 0);
    assert(ca_rt_session_receive_event_next(s, &e));
    assert(e.type == CA_RT_EVT_SPEECH_ENDED);
    ca_rt_event_free(&e);
    assert(ca_rt_session_receive_audio_next(s, &out)); ca_rt_audio_frame_free(&out);

    /* Short frame (<64 bytes) counts as silent (no transition from silent). */
    uint8_t tiny[16]; fill_loud(tiny, sizeof(tiny));
    ca_rt_audio_frame_t tf; memset(&tf, 0, sizeof(tf));
    tf.pcm = tiny; tf.pcm_len = sizeof(tiny); tf.format = CA_RT_FMT_PCM24K;
    assert(ca_rt_session_send_audio(s, &tf) == 0);
    assert(ca_rt_session_event_pending(s) == 0);
    assert(ca_rt_session_receive_audio_next(s, &out)); ca_rt_audio_frame_free(&out);

    ca_rt_session_destroy(s);
...
static void test_loopback_text(void) {
    ca_rt_session_config_t c = cfg(CA_RT_FMT_PCM16K);
    ca_rt_session_t *s = ca_rt_loopback_session_create(&c, NULL, NULL);
    assert(s);

    /* "hello world foo" = 3 words -> durMs = max(50, 240) = 240ms.
     * sr=16000 -> sampleCount = 16000*240/1000 = 3840 -> 7680 bytes. */
    assert(ca_rt_session_send_text(s, "hello world foo") == 0);

    /* one outbound audio frame carrying the silence PCM. */
    assert(ca_rt_session_audio_pending(s) == 1);
    ca_rt_audio_frame_t f;
    assert(ca_rt_session_receive_audio_next(s, &f));
    assert(f.pcm_len == 7680);
    assert(f.format == CA_RT_FMT_PCM16K);
    assert(f.offset_ticks == 0);   /* first frame at offset 0 */
    /* silence: all zeros. */
    for (size_t i = 0; i < f.pcm_len; ++i) assert(f.pcm[i] == 0);
    ca_rt_audio_frame_free(&f);

    /* events: TranscriptDelta(Outbound), TranscriptFinal(Outbound), TurnComplete. */
    ca_rt_event_t evs[8];
    size_t n = drain_events(s, evs, 8);
    assert(n == 3);
    assert(evs[0].type == CA_RT_EVT_TRANSCRIPT_DELTA &&
           evs[0].direction == CA_RT_DIR_OUTBOUND &&
           strcmp(evs[0].text, "hello world foo") == 0);
    assert(evs[1].type == CA_RT_EVT_TRANSCRIPT_FINAL &&
           evs[1].direction == CA_RT_DIR_OUTBOUND &&
           strcmp(evs[1].text, "hello world foo") == 0);
    assert(evs[2].type == CA_RT_EVT_TURN_COMPLETE);
    for (size_t i = 0; i < n; ++i) ca_rt_event_free(&evs[i]);
    ...
    assert(ca_rt_session_send_text(s, "hi") == 0);
    assert(ca_rt_session_receive_audio_next(s, &f));
    assert(f.pcm_len == 2560);
    assert(f.offset_ticks == 2400000LL);
    ca_rt_audio_frame_free(&f);
    /* drain its 3 events. */
    ...
    assert(ca_rt_session_send_text(s, "") == 0);
    assert(ca_rt_session_receive_audio_next(s, &f));
    ...
    /* NULL text -> error. */
    assert(ca_rt_session_send_text(s, NULL) == -1);
...
static void test_loopback_tool_and_cancel(void) {
    ...
    assert(ca_rt_session_send_tool_result(s, "call-1", "{\"ok\":true}") == 0);
    ca_rt_event_t e;
    assert(ca_rt_session_receive_event_next(s, &e));
    assert(e.type == CA_RT_EVT_TRANSCRIPT_DELTA && e.direction == CA_RT_DIR_OUTBOUND);
    ...
    big[100] = '\0';
    assert(ca_rt_session_send_tool_result(s, "c2", big) == 0);
    assert(ca_rt_session_receive_event_next(s, &e));
    ...
    assert(ca_rt_session_send_tool_result(s, "  ", "x") == -1);
    assert(ca_rt_session_send_tool_result(s, "c", NULL) == -1);

    /* Cancel -> TurnComplete. */
    assert(ca_rt_session_cancel_response(s) == 0);
    assert(ca_rt_session_receive_event_next(s, &e));
    assert(e.type == CA_RT_EVT_TURN_COMPLETE);
    ca_rt_event_free(&e);
...
static int fixed_tts(void *ctx, const char *text, ca_rt_audio_format_t fmt,
...
static void test_custom_tts(void) {
    ca_rt_session_config_t c = cfg(CA_RT_FMT_PCM24K);
    ca_rt_session_t *s = ca_rt_loopback_session_create(&c, fixed_tts, NULL);
    assert(s);
    assert(ca_rt_session_send_text(s, "anything") == 0);
    ca_rt_audio_frame_t f;
    assert(ca_rt_session_receive_audio_next(s, &f));
...
static void test_null_session(void) {
    ca_rt_session_t *s = ca_rt_null_session_create();
    assert(s);
    assert(strcmp(ca_rt_session_id(s), "null") == 0);

    uint8_t buf[128] = {0};
    ca_rt_audio_frame_t f; memset(&f, 0, sizeof(f));
    f.pcm = buf; f.pcm_len = sizeof(buf); f.format = CA_RT_FMT_PCM24K;
    assert(ca_rt_session_send_audio(s, &f) == 0);
    assert(ca_rt_session_send_text(s, "hi") == 0);
    assert(ca_rt_session_send_tool_result(s, "c", "{}") == 0);
    assert(ca_rt_session_cancel_response(s) == 0);
    /* both streams yield nothing. */
    assert(ca_rt_session_audio_pending(s) == 0);
    assert(ca_rt_session_event_pending(s) == 0);
    ca_rt_audio_frame_t o; ca_rt_event_t e;
    assert(!ca_rt_session_receive_audio_next(s, &o));
    assert(!ca_rt_session_receive_event_next(s, &e));
    ca_rt_session_destroy(s);
...
static void test_services(void) {
    /* Loopback service. */
    ca_rt_service_t *lp = ca_rt_loopback_service_create(NULL, NULL);
    assert(lp);
    assert(strcmp(ca_rt_service_provider_id(lp), "loopback") == 0);
    assert(ca_rt_service_is_configured(lp));

    ca_rt_session_config_t c = cfg(CA_RT_FMT_PCM24K);
    ca_rt_session_t *s = ca_rt_service_start_session(lp, &c);
    assert(s);
    assert(strncmp(ca_rt_session_id(s), "loop-", 5) == 0);
    /* session works end to end. */
    assert(ca_rt_session_send_text(s, "one two") == 0);
    assert(ca_rt_session_audio_pending(s) == 1);
    ca_rt_session_destroy(s);

    /* start with NULL config -> NULL. */
    assert(ca_rt_service_start_session(lp, NULL) == NULL);
    ca_rt_service_destroy(lp);

    /* Null service. */
    ca_rt_service_t *nul = ca_rt_null_service_create();
    assert(nul);
    assert(strcmp(ca_rt_service_provider_id(nul), "null") == 0);
    assert(!ca_rt_service_is_configured(nul));
    /* StartSession refuses (C# throws) -> NULL. */
    assert(ca_rt_service_start_session(nul, &c) == NULL);
    ca_rt_service_destroy(nul);
    printf("  services: ok\n");
}
...
static void test_null_transport_factory(void) {
    ca_rt_transport_factory_t f = ca_rt_null_transport_factory();
    assert(f.connect);
    ca_rt_transport_t t;
