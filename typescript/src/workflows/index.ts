...
// ═════════════════════════════════════════════════════════════════════════════
// PACA modules (paca port) — per-file modules, re-exported here as the barrel.
// ═════════════════════════════════════════════════════════════════════════════

// Conversation state machine + OpenHands runtime contract (PacaConversations.cs).
export {
  ConversationState,
  conversationStep,
...
// Project + task primitives (PacaProjects.cs).
export { InMemoryPacaStore, taskReference } from "./paca_projects.js";
export type { PacaProject, PacaTask } from "./paca_projects.js";

// Sprintboard surface (PacaBoards.cs).
export { SprintState, statusColumn, PacaBoard } from "./paca_boards.js";
export type { StatusColumn, PacaSprint, TaskBoardMetadata, BoardView } from "./paca_boards.js";

// Auth: HMAC-SHA256 JWT + API keys (PacaAuth.cs).
export { HmacJwtAuthenticator, PacaApiKeyAuthenticator } from "./paca_auth.js";
export type { JwtPair, JwtPayload, PacaApiKeyRecord, IssuedApiKey } from "./paca_auth.js";

// AI agents as project members + 5 preset templates (PacaAgents.cs).
export { MemberKind, AgentTemplates, InMemoryPacaMemberStore } from "./paca_agents.js";
export type {
  ProjectMember,
  AgentLlmConfig,
  AgentSystemPrompts,
...
// Living documents with versions, activity, links, @mentions (PacaDocs.cs).
export { PacaDocService } from "./paca_docs.js";
export type { DocNode, DocVersion, DocActivity, DocLink, DocDiff } from "./paca_docs.js";

// Plugin runtime + manifest + lifecycle (PacaPlugins.cs).
export { PluginExtensionPoint, pluginResourceLimits, PacaPluginRegistry } from "./paca_plugins.js";
export type { PluginResourceLimits, PluginManifest, InstalledPlugin, IPluginRuntimeHost } from "./paca_plugins.js";

// MCP server: tool registry + per-agent config (PacaMcp.cs).
export { McpTransportKind, pacaMcpTool, PacaMcpServer, PacaCoreMcpTools } from "./paca_mcp.js";
export type { AgentMcpConfig, PacaMcpTool, PacaMcpHandler } from "./paca_mcp.js";

// Realtime fan-out: permission-aware rooms + query invalidation (PacaRealtime.cs).
export {
  RealtimePacaEventKind,
  taskUpdatedEvent,
...
// Eleven built-in Claude Code skills + installer (PacaSkills.cs).
export {
  pacaSkill,
  skillToMarkdown,
  skillToBodyOnly,
...
export type { PacaSkill, ISkillFileStore } from "./paca_skills.js";

// Single-command install: docker compose + .env generation (PacaDeploy.cs).
export { PacaDeployMode, pacaDeployOverrides, PacaDeployer } from "./paca_deploy.js";
export type { PacaDeployOverrides, PacaDeployArtifact } from "./paca_deploy.js";
